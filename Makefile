...
export-seed:
	@if [ -z "$(DB_PASSWORD)" ]; then echo "DB_PASSWORD is required"; exit 1; fi
	go run scripts/export_seed_data.go -host=${DB_HOST} -port=${DB_PORT} -user=${DB_USER} -password=${DB_PASSWORD} -dbname=${DB_NAME} -sslmode=${DB_SSLMODE}

export-seed-docker:
	@if [ -z "$(DB_PASSWORD)" ]; then echo "DB_PASSWORD is required"; exit 1; fi
	docker exec -it ${CONTAINER_NAME} /bin/sh -c "go run scripts/export_seed_data.go -host=${DB_HOST} -port=${DB_PORT} -user=${DB_USER} -password=${DB_PASSWORD} -dbname=${DB_NAME} -sslmode=${DB_SSLMODE}"
